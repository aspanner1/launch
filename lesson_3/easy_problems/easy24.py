...
determine which value occurs twice. Write a function that finds and returns the duplicate value.
...
Iterate through input list 
    If input is not in empty list, add it 
    If it is, return it
...
def find_dup(int_list):
    ...
    for number in int_list:
        if(number not in storage_list):
            storage_list.append(number)
        else:
            return number
